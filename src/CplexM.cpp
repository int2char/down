                    ...
                    IloNum,         startDetTime)
{
   ...
   if ( hasIncumbent()) {
	  middata.push_back(getIncumbentObjValue());
   }

}

CplexM::CplexM(vector<service>&ser, taskPath*_PathSets, Graph &G, ofstream&_Out):Gm(G),serv(ser),Source(Task,std::vector<int>()),constraint(EDge,vector<pair<float,int>>())
{
	...
	for(int i=0;i<Task;i++)
	{
		...
		for(int j=0;j<n;j++)
		{
			...
			Source[i].push_back(vcount);
			vbw.push_back(ser[i].d);
			va2d[vcount]=i;
			va2p[vcount]=j;
...
pair<int,vector<pair<string,float>>> CplexM::solve(){
	...
	for(int i=0;i<variables.size();i++)
		{
		x.add(IloNumVar(env,0,vbw[i],IloNumVar::float));
		}
	...
	for(int i=0;i<constraint.size();i++)
		for(int j=0;j<constraint[i].size();j++)
			con[i].setLinearCoef(x[constraint[i][j].second],1);
